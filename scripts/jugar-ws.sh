...
vigilante() {
  ...
  while pgrep -x 'nfs4ws.exe' >/dev/null 2>&1; do
    ...
    if [ "$armado" -eq 1 ] && [ "$negras" -ge 4 ]; then
      pkill -x 'nfs4ws.exe' 2>/dev/null
      sleep 2
      pgrep -x 'nfs4ws.exe' >/dev/null && pkill -9 -x 'nfs4ws.exe' 2>/dev/null
...
wine nfs4ws.exe "$@"          # sin exec: hay que volver aquí para limpiar

# --- Limpieza ---------------------------------------------------------------------
kill "$VIGIA" 2>/dev/null
sleep 1
pgrep -x 'nfs4ws.exe' >/dev/null && { pkill -x 'nfs4ws.exe'; sleep 1; }
wineserver -k 2>/dev/null
